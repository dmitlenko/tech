...
class MainWindow(Tk):
    ...
    def __init__(self,parent):
        ...
        self._do()
        self.tut.lift()
        self.tut.focus_force()
        self.tut.grab_set()
        self.tut.grab_release()
    ...
    def _do(self):
        ...
        def drawSin():
            xy1 = []
            fun = { 0:lambda x: sin(x), 1:lambda x: cos(x), 2:lambda x: tan(x) }[funcType.get()]
            for x in range(self.width):
                xy1.append(x * self.x_inc)
                xy1.append(int(fun((x * self.x_fac) + self.x_bias) * self.y_amp) + self.center)
        ...
        def showJson():
            ...
            try:
                tx1.txt.insert(0.0,dumps(self.dots))
            except:
                top.withdraw()
        ...
        c1.grid(column=0,row=3,sticky=NW)
        c2.grid(column=0,row=4,sticky=NW)
        Label(controlFrame,text='Функція:').grid(column=0,row=5,sticky=NW)
        r1.grid(column=0,row=6,sticky=NW)
        r2.grid(column=0,row=7,sticky=NW)
        r3.grid(column=0,row=8,sticky=NW)
        Label(controlFrame,text='Точки:').grid(column=0,row=9,sticky=NW)
        lb1.grid(column=0,row=10,sticky=NW)
        b3.grid()
        b2.grid()
        b1.grid()
